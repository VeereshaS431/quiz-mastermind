...
const CssData = () => {
    ...
    const fetchData = async () => {
        const dbRef = ref(realtimeDb, "quize/css");
    ...
    const saveQuizResult = async () => {
        ...
        const resultData = {
            currect: correct,
            wrong,
            unanswered: randomData.length - (correct + wrong),
            percentage: (correct / randomData.length) * 100,
                    ...
                    <Container maxWidth="sm" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '80px' }}>
                        <Box sx={{ background: "rgb(2,0,36)", background: "linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(23,57,137,1) 29%, rgba(0,212,255,1) 100%)", width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', rowGap: "20px", flexDirection: 'column', borderRadius: '10px', p: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', height: '30%', rowGap: "5px" }}>
                                <Typography textAlign={"center"} color={"white"}>Correct Answers: {correct}</Typography>
                                <Typography textAlign={"center"} color={"white"}>Wrong Answers: {wrong}</Typography>
                                <Typography textAlign={"center"} color={"white"}>Unanswered Questions: {randomData.length - (correct + wrong)}</Typography>
                                <Typography textAlign={"center"} color={"white"}>Percentage: {(correct / randomData.length) * 100} %</Typography>
                            </Box>
                            <Button onClick={() => {
                                setIndex(0);
                                update(0);
                ...
                <>
                    {startButton && (
                        <>
                            ...
                            <Container maxWidth="sm" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '80px' }}>
                                <Box sx={{ background: "rgb(2,0,36)", background: "linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(23,57,137,1) 29%, rgba(0,212,255,1) 100%)", width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', rowGap: "20px", flexDirection: 'column', borderRadius: '10px', p: 2 }}>
                                    <Typography marginTop={5} textAlign={"center"} color={"white"} fontWeight={"bold"} variant="h4">Start The Quiz of CSS</Typography>
